...
spl_autoload_register(function ($name) {
    if (preg_match('/_/', $name)) {
        $name = preg_replace('/_/', '/', $name);  // tmp_passwd → /tmp/passwd
    }
    $filename = "/${name}.php";
...
''',
    ...
    def detect_from_finding(self, finding: Dict[str, Any], profile: Optional['TargetProfile'] = None) -> float:
        """Activate on deserialization vulnerability detection"""
        ...
        if finding_type == FindingTypes.DESERIALIZATION_VULN:
            logger.info("Deserialization attacks activating: Deserialization vulnerability detected")
            return 100

        # High - Deserialization indicators
        deser_indicators = ['deserialization', 'unserialize', 'pickle', 'java serialization',
                           'ysoserial', 'marshalling', 'unmarshalling']
        if any(ind in description for ind in deser_indicators):
            logger.info("Deserialization attacks activating: Deserialization indicator detected")
            return 90

        # Medium - Serialization libraries/frameworks
        frameworks = ['jackson', 'gson', 'fastjson', 'xstream', 'yaml', 'pyyaml']
        if any(fw in description for fw in frameworks):
            logger.info("Deserialization attacks activating: Serialization framework detected")
            return 75
