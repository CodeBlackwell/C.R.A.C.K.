...
class FTPPlugin(ServicePlugin, SQLPluginMixinV2):
    """FTP enumeration plugin with SQL backend support"""
    ...
    def get_task_tree(self, target: str, port: int, service_info: Dict[str, Any]) -> Dict[str, Any]:
        """
        Generate FTP enumeration task tree (SQL-backed with fallback).

        Tries SQL backend first. If unavailable (DB missing, migration not applied),
        falls back to hardcoded tasks for backwards compatibility.
        """
        try:
            return self.get_task_tree_from_sql(target, port, service_info)
        except Exception as e:
            # Log warning (not error - fallback is expected behavior)
    ...
    def _get_hardcoded_task_tree(self, target: str, port: int, service_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        This is the EXACT COPY of the original get_task_tree() implementation.
        """
