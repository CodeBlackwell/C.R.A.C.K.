...
class SSRFAttacksPlugin(ServicePlugin):
    ...
    def detect_from_finding(self, finding: Dict[str, Any], profile: Optional['TargetProfile'] = None) -> float:
        ...
        # Perfect match
        if finding_type == FindingTypes.SSRF_FOUND:
            logger.info("SSRF attacks activating: SSRF vulnerability detected")
            return 100

        # High - SSRF indicators
        if 'ssrf' in description or 'server-side request forgery' in description:
            logger.info("SSRF attacks activating: SSRF keyword detected")
            return 90

        # Medium - SSRF-prone patterns
        ssrf_patterns = ['url parameter', 'fetch url', 'webhook', 'callback url',
                        'proxy parameter', 'redirect url']
        if any(pattern in description for pattern in ssrf_patterns):
            logger.info("SSRF attacks activating: SSRF-prone pattern detected")
            return 70
