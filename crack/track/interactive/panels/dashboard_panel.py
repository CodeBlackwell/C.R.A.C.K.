...
class DashboardPanel:
    ...
    @classmethod
    def render(cls, profile, recommendations: Dict[str, Any]) -> Tuple[Panel, List[Dict]]:
        """
        ...
        if next_task:
            table.add_row(cls._render_recommendation_card(next_task))
            table.add_row("")  # Blank line

        # 2. Action Menu
        choices = cls._build_action_menu(profile, recommendations, table)
    ...
    @classmethod
    def _render_recommendation_card(cls, task) -> str:
        """
        ...
        if 'QUICK_WIN' in tags:
            badges.append('[bright_yellow]⚡ QUICK WIN[/]')
        if 'OSCP:HIGH' in tags:
            badges.append('[bright_red]🎯 OSCP HIGH[/]')
        if 'OSCP:MEDIUM' in tags:
            badges.append('[yellow]🎯 OSCP MED[/]')

        badge_str = ' '.join(badges) if badges else '[dim]Standard Priority[/]'
        ...
        # Build card
        card_lines = [
            "[bold bright_cyan]🎯 NEXT RECOMMENDED TASK[/]",
            "┌─────────────────────────────────────────────────────────────────────────┐",
            f"│ [bold]{task.name}[/]",
            f"│ [dim]{description}[/]",
            f"│",
            f"│ {badge_str} | [cyan]Time:[/] ~{time_est}",
        ]

        # Add command preview if available
        if cmd_preview:
            card_lines.append(f"│ [dim]Command:[/] [bright_black]{cmd_preview}[/]")
    ...
    @classmethod
    def _build_action_menu(cls, profile, recommendations: Dict[str, Any], table: Table) -> List[Dict]:
        """
        ...
        if next_task:
            table.add_row(f"[bold bright_white]{menu_num}.[/] Execute next recommended task")
            choices.append({
                'id': 'next',
        ...
        table.add_row(f"[bold bright_white]{menu_num}.[/] Browse all tasks [dim]({task_count} available)[/]")
        choices.append({
            'id': 'browse-tasks',
        ...
        if quick_wins:
            ...
            table.add_row(f"[bold bright_white]{menu_num}.[/] Quick wins ⚡ [dim]({qw_count} available)[/]")
            choices.append({
                'id': 'quick-wins',
        ...
        table.add_row(f"[bold bright_white]{menu_num}.[/] Import scan results")
        choices.append({
            'id': 'import',
        ...
        # 5. Document finding
        table.add_row(f"[bold bright_white]{menu_num}.[/] Document finding")
        choices.append({
            'id': 'finding',
        ...
        table.add_row(f"[bold bright_white]{menu_num}.[/] Browse findings [dim]({finding_count} total)[/]")
        choices.append({
            'id': 'browse-findings',
        ...
        # 7. Full status
        table.add_row(f"[bold bright_white]{menu_num}.[/] Full status")
        choices.append({
            'id': 'show-status',
            'label': 'Full status'
        })
        ...
        # 8. Help
        table.add_row(f"[bold bright_white]{menu_num}.[/] Help")
        choices.append({
            'id': 'help',
            'label': 'Help'
        })
        ...
        # 9. Exit
        table.add_row(f"[bold bright_white]{menu_num}.[/] Exit")
        choices.append({
            'id': 'exit',
            'label': 'Exit'
        })
    ...
    @classmethod
    def render_empty_state(cls, profile) -> Tuple[Panel, List[Dict]]:
        """
        ...
        # Empty state message
        table.add_row("[bold yellow]👋 Welcome to CRACK Track TUI![/]")
        table.add_row("")
        table.add_row("[dim]No tasks available yet. Import a scan file to get started.[/]")
        table.add_row("")
