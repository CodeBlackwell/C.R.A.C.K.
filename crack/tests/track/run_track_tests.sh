...
# User-story driven testing for the track checklist tool
#
...
case $COMMAND in
    all)
        echo -e "${YELLOW}Running ALL track tests...${NC}"
        pytest tests/track/ -v --tb=short
        ;;

    user-stories)
        echo -e "${YELLOW}Running USER STORY tests (real-world workflows)...${NC}"
        pytest tests/track/test_user_stories.py -v
        ;;

    guidance)
        echo -e "${YELLOW}Running GUIDANCE QUALITY tests...${NC}"
        pytest tests/track/test_guidance_quality.py -v
        ;;

    edge-cases)
        echo -e "${YELLOW}Running EDGE CASE tests...${NC}"
        pytest tests/track/test_edge_cases.py -v
        ;;

    docs)
        echo -e "${YELLOW}Running DOCUMENTATION tests...${NC}"
        pytest tests/track/test_documentation.py -v
        ;;
    ...
    fast)
        ...
        pytest tests/track/ -v -m "not slow" --tb=line
        ;;
    ...
    story)
        ...
        pytest tests/track/test_user_stories.py::TestUserStory${2}_* -v
        ;;

    debug)
        echo -e "${YELLOW}Running tests in DEBUG mode...${NC}"
        pytest tests/track/ -v -s --tb=long
        ;;
    ...
    watch)
        ...
        pytest-watch tests/track/ -- -v --tb=short
        ;;
    ...
    clean)
        ...
        rm -f .coverage
        rm -f coverage.xml
        find tests/track -type d -name __pycache__ -exec rm -rf {} + 2>/dev/null || true
    ...
    help|--help|-h)
        ...
        echo ""
        echo "Commands:"
        echo "  all           - Run all track tests (default)"
