"""
Theme Manager - Centralized theme loading and color access for CRACK

CRACK Core Theme System - All modules can use this

Handles:
- Loading theme from config (~/.crack/config.json)
- Switching themes dynamically
- Accessing semantic colors (primary, success, warning, danger, etc.)
- Accessing component-specific colors

Example:
    from crack.themes import ThemeManager

    theme = ThemeManager()
    color = theme.get_color('primary')  # Returns 'cyan' for OSCP theme
    theme.set_theme('dark')  # Switch to dark theme
"""
...
# Optional debug logger support (graceful fallback)
try:
    from crack.track.interactive.log_types import LogCategory, LogLevel
except ImportError:
    # Graceful fallback if track module not available
    LogCategory = None
    LogLevel = None


class ThemeManager:
    """Central theme management system for CRACK"""
    ...
    def __init__(self, config_path: Optional[str] = None, debug_logger=None):
        ...
        Args:
            ...
            debug_logger: Optional DebugLogger instance for logging (from track module)
        """
    ...
    # Convenience methods for common colors (Rich markup format)
